...
export class Stage {
    ...
    spawnEnemy() {
        ...
        const enemies = entityManager.getEnemies();
        if (!this.tanks.length || enemies.length > 4 || enemySpawnCD)
            return;
        const index = randomInt(2);
        this.timeManager.setTimer('enemySpawnCD', ENEMY_SPAWN_CD);
        entityManager.spawnEntity('Enemy', this.tanks.pop(), Object.assign({}, ENEMY_SPAWN_POSITION[index]));
    }
    spawnPowerup() {
        const powerupSpawnCD = this.timeManager.getTimer('powerupSpawnCD');
        if (powerupSpawnCD || !this.powerupsAvailable)
            return;
