...
function powerupObserver(powerupType) {
    if (powerupType === PowerupTypes.Stopwatch) {
        ...
        if (!this.timeManager.some(['death', 'spawn']))
            this.die();
    }
}
export class Enemy extends Tank {
    constructor(type, position) {
        super(position, Object.assign({}, TANK_SIZE), Direction.Bottom);
        this.type = type;
        this.lives = ENEMY_STATS[type].lives;
        this.prevTile = Object.assign({}, position);
    ...
    aiMove() {
        ...
        if (Math.abs(Math.floor(this.prevTile.x - this.position.x)) > 120 ||
            Math.abs(Math.floor(this.prevTile.y - this.position.y)) > 120) {
            this.prevTile = Object.assign({}, this.position);
    ...
    resolveEntityCollision(other) {
        if (isPowerup(other)) {
            ...
            if (this.timeManager.getTimer('shotCD') > 10)
                this.setOpositeDirection();
            this.goBack();
        }
    }
    setRandomDirection() {
        const items = [Direction.Top, Direction.Right, Direction.Bottom, Direction.Left].filter(direction => direction !== this.direction);
